...
export default function DashBoardAdmin(
  ...
  let loggedInUserID = IWfDashboardProps.context.pageContext.legacyPageContext["userId"];
  /*--------------------------------------ButtonIcon---------------------------------*/
  ...
  return (
    <div style={{ margin: "1rem 2rem" }}>
      ...
      >
        <div className="DialogBtn" style={{ marginBottom: "2rem",  position: "relative" }}>
        <TextField
            style={{
              position: "absolute",
              height: "0px",
          ...
          />
          <SharedWithPeople peoples={users} update={updateSharedWithID} GetUserDetails={GetsharedwithUserDetails}/>
          {/*<PeoplePicker
  ...
  function doesTextStartWith(text: string, filterText: string): boolean {
    ...
    {
      await IWfDashboardProps.spcontext.siteGroups.getByName('AdminGroup').users.top(5000).get().then(async function(result) 
      {
    ...
    async function getGroupFromList()
    {
        var groups=[];
        var usersdata=[];
        await IWfDashboardProps.spcontext.lists.getByTitle("ConfigUsers")
           ...
           if(data.length>0)
           {
                await data.forEach(async item => 
                {
                    if(item.GroupName.length>0)
                    {
                        ...
                        {
                            await IWfDashboardProps.spcontext.siteGroups.getById(groupid).users.get().then(async function(result) 
                                {
